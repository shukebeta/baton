...
The current blessed release is `v0.2.11`.
...
```bash
cargo install --git https://github.com/shukebeta/baton --tag v0.2.11 --locked
```
...
| `BATON_EVENT_LOG`            | no       | — (disabled)                | File path for the JSONL exchange-event trail, opened in append mode. Also carries the `baton session` [session trail](docs/protocol.md#session-trail) (session start/end markers + per-turn `session_id` / `turn_index`). See below. |
...
[`baton log merge`](docs/protocol.md#cross-trail-merge-baton-log-merge).
...
- History lives only in memory: it is not persisted across process restarts.
  ...
  into whole sessions (see [Session trail](docs/protocol.md#session-trail)).
...
Baton can record each exchange as a machine-readable JSONL trail (`baton.exchange/v1`)
so a single request/response can be programmatically inspected or replayed.
Recording is **opt-in** via `BATON_EVENT_LOG`. The full event schema — line
format, the `request`/`response_ok`/`response_error`/`session_start`/`session_end`
events, optional token counts, failure `kind` classes, and the partial-line
consumption model — lives in the protocol reference:

- [docs/protocol.md → Structured exchange events](docs/protocol.md#structured-exchange-events)
- [docs/protocol.md → Session trail](docs/protocol.md#session-trail) — `session_id` / `turn_index` framing.

## Inspecting and replaying the trail

`baton log` makes the recorded trail a first-class artefact: `baton log show`
pretty-prints each exchange, `baton log replay [--index <N>]` re-runs a recorded
request with the current credential, and `baton log merge --conversation <id>`
reunites `baton.message/v1` envelopes split across async trails into one
causally-ordered view. Command contracts and the merge ordering rules are in the
reference:

- [docs/protocol.md → Inspecting and replaying the trail](docs/protocol.md#inspecting-and-replaying-the-trail)
- [docs/protocol.md → Cross-trail merge](docs/protocol.md#cross-trail-merge-baton-log-merge)

## A2A message envelope (`baton.message/v1`)

`baton.message/v1` is the agent-to-agent *peer message* envelope shared by the
exchange verb and driver — a contract only, carrying no transport or addressing
semantics. It nests zero-or-one `baton.exchange/v1` provider-call record under
`exchange`. The full field table, `kind` variants, and the nesting shape are in
the reference:

- [docs/protocol.md → A2A message envelope](docs/protocol.md#a2a-message-envelope-batonmessagev1)
- [docs/protocol.md → Nesting over `baton.exchange/v1`](docs/protocol.md#nesting-over-batonexchangev1)
